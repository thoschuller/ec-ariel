...
from utils import plot_saved_phenotype, plot_and_record_saved_phenotype, run_saved_phenotype
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Plot, record, or evolve Silly Walkers.")
    ...
    parser.add_argument(
        "--duration", type=float, default=default_duration, help="Duration of the simulation."
    )
    parser.add_argument(
        "--video",
        action="store_true",
    ...
    try:       
        ...
        if [args.evolve, args.viewer, args.plot, args.video].count(True) > 1:
            raise ValueError("Only one of --evolve, --viewer, --plot, or --video can be set at a time.")
        ...
        if args.viewer:
            if not args.brain or not args.body:
                console.log("[red] [ERROR] --brain and --body must be set for viewer mode.")
                sys.exit(1)
