...
# --- Helpers ---
def run(cmd: list[str]) -> subprocess.CompletedProcess: # pyright: ignore[reportUnknownParameterType, reportMissingTypeArgument]
...
def ffprobe_audio_codec(path: Path) -> str | None:
    cmd = [
        "ffprobe",
        "-v",
        "error",
...
def parse_codec_list(s: str | None) -> set[str]:
    if not s:
        return set()
...
def swap_ext_keep_last_dot(
    p: Path, new_suffix: str, outdir: Path | None = None
) -> Path:
    name = p.name
...
def build_ffmpeg_cmd(
    ...
    if passes == 1:
        ...
        cmd1 = (
            ...
            + ["-an"]
            + vopts
            + ["-pass", "1", "-passlogfile", str(passlog), "-f", "mp4", os.devnull]
        )
        cmd2 = (
            common
            + vopts
            + aopts
            + ["-pass", "2", "-passlogfile", str(passlog), str(dst)]
        )
        return [cmd1, cmd2]
...
class Handler(FileSystemEventHandler):
    ...
    def on_created(self, event): # pyright: ignore[reportMissingParameterType]
    ...
    def on_moved(self, event): # pyright: ignore[reportMissingParameterType]
...
def main():
    ...
    ap.add_argument(
        ...
        choices=["hevc", "av1"],
        default="hevc",
        help="Target codec (default: hevc)",
    )
    ap.add_argument(
        "--outdir",
        type=str,
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="Speed preset (x265: ultrafast..placebo; SVT-AV1: 0..13; libaom: use digits 0..8)",
    )
    ap.add_argument(
        "--two-pass",
        action="store_true",
        help="2-pass for libaom-av1 (ignored for HEVC/SVT-AV1)",
    )
    ap.add_argument(
        "--scan-existing",
        action="store_true",
        help="Also process existing files at startup",
    )
    ap.add_argument(
        "--only-codecs",
        type=str,
    ...
    if not shutil.which("ffmpeg") or not shutil.which("ffprobe"):
        print(
            "ERROR: ffmpeg and ffprobe must be installed and on PATH.", file=sys.stderr
        )
        sys.exit(2)

    print(f"[start] Watching: {watch_dir}")
    print(
        f"[config] target={args.to} outdir={outdir or '(same as source)'} crf={args.crf} preset={args.preset} two_pass={args.two_pass}"
    )
    if only_codecs:
    ...
    event_handler = Handler(
        args.to, outdir, args.crf, args.preset, args.two_pass, only_codecs
    )
    observer = Observer()
